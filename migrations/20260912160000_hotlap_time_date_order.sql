-- Remove splits from personal-best selection and chart tie-breaks.
-- Select the earliest upload when a player's lap times are equal.
WITH best AS (
...
WITH positions AS (
    SELECT pb.hotlap_id,
        ROW_NUMBER() OVER (
            ...
            ORDER BY hotlap.lap_time_ms, hotlap.created_at,
